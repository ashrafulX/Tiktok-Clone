...
from posts.models import LikedPost, LikedComment, Comment, Repost
from django.db.models import Q
from itertools import chain
from operator import attrgetter
from .models import NotificationTracker
from messages.models import ConvUser
...
def notifications(request):
    ...
    likedposts = LikedPost.objects.filter(
    ...
    combinednotifications = sorted(
        chain(followers, likedposts, liked_comments, comments, replies, reposts),
        key=attrgetter('created_at'),
        reverse=True
    )
    
    notifications = combinednotifications[:20]
...
def newnotifications(request):
    ...
    has_newnotifications = True
    
    if last_seen:
        has_newnotifications = (
    ...
    has_newmessages = ConvUser.objects.filter(user=request.user, unread_count__gt=0).exists()
    
    context = {
        'has_newnotifications': has_newnotifications,
        'has_newmessages': has_newmessages,
    }
    
    
    return render(request, 'notifications/notify_dot.html', context)